...
const checarAnoBissexto = (ano) => {
    let status = false;
    (ano % 400 == 0 && ano % 100 == 0 && ano % 4 == 0) ? status = true : status = false;
    (status == true || ano % 100 != 0 && ano % 4 == 0) ? status = true : status = false;
    (status == true || ano % 4 == 0) ? status = true : status = false;
    return console.log("Ano : " + ano + " - " + status)
}

checarAnoBissexto(2020)
checarAnoBissexto(2019)
checarAnoBissexto(2018)
checarAnoBissexto(2017)
checarAnoBissexto(2016)
checarAnoBissexto(2015)
checarAnoBissexto(2000)